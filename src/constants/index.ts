
export const shifts = ["Day", "Night", "Float"];

export const leadershipTypes = [
  "Travel",
  "Per Diem",
  "Permanent",
  "Interim Leadership"
];

export const professionsList = [
  "RN",
  "LPN/LVN",
  "CNA",
  "Occupational Therapy",
  "Pharmacy",
  "Physicist",
  "Respiratory Therapy",
  "Sitter",
  "Social Services",
  "Sports Medicine",
  "Surgical Services",
  "Radiology/Imaging",
  "Audiologist",
  "Perfusionist",
  "Physical Therapy",
  "Speech Therapy",
  "Health and Information Management",
  "Environmental Services",
  "Finance/Accounting",
  "Food and Nutrition Services",
  "Human Resources",
  "Dialysis Tech",
  "Advanced Practice Registered Nurse",
  "Laboratory",
  "Behavioral Health",
  "Cardiology",
  "Chiropractor",
  "Clinical Support Tech",
  "Dentistry",
  "Dietary Services",
  "EMT",
  "ER Tech",
  "Medical Assistant",
  "Neuro Diagnostics",
  "Ophthalmology",
  "Paramedic",
  "Patient Care Technician",
  "Certified Surgical Tech",
  "Ultrasound Tech",
  "CMA",
  "Sterile Processing Tech",
  "MRI Tech",
  "Cath Lab Technologist",
  "CT Technologist",
  "SLP",
  "LTC",
  "Cardiac Tele",
  "Social Worker",
  "Pharmacist",
  "Scrub Tech",
  "Mother Baby",
  "Unit Manager",
  "Endoscopy Tech",
  "Sonographer",
  "Monitor Tech",
  "Radiation Therapist",
  "Physician",
  "PTA",
  "Phlebotomist",
  "Mammography Technologist",
  "Dosimetrist",
  "Pharmacy Tech",
  "Genetic Counselor",
  "Surgical Assistant",
  "Lab Specialist"
];

export const specialtiesMap = {
  "RN": [
    "Registered Nurse",
    "Acute Care",
    "Pediatrics",
    "Geriatrics",
    "Acute Rehab",
    "Ambulatory",
    "Case Manager",
    "Corrections",
    "COVID",
    "CVICU",
    "Educator",
    "Float Pool",
    "Hospice",
    "ICU",
    "Infection Control",
    "Infusion",
    "Long Term Care",
    "Med Surg",
    "Outpatient Clinic",
    "Oncology",
    "Psych",
    "Public Health",
    "School",
    "Urgent Care",
    "Urology",
    "Vaccinator",
    "Vascular Surgery",
    "Triage",
    "PACU",
    "Pre-Op/Post-Op/Same Day Surgery",
    "Transplant",
  ],

  "LPN/LVN": [
    "LPN",
    "LPN/LVN",
  ],

  "Advanced Practice Registered Nurse": [
    "Registered Nurse First Assist (RNFA)",
    "Certified Registered Nurse Anesthetist (CRNA)",
    "Certified Nurse Midwife",
    "Nurse Practitioner",
    "Clinical Nurse Specialist",
  ],

  "LTC": [
    "LTAC",
    "LTC",
    "Long Term Care",
    "SNF",
    "Sub Acute",
  ],

  "Dialysis Tech": [
    "Dialysis",
    "Dialysis Tech",
  ],

  "ER Tech": [
    "Emergency Room",
    "ER Tech",
    "Pediatric ER",
  ],

  "Endoscopy Tech": [
    "GI/Endoscopy",
    "GR / Endo Tech",
    "Endoscopy Tech",
  ],

  "Unit Manager": [
    "House Supervisor",
    "Manager",
    "Cath Lab Manager",
    "Radiology Manager",
    "Unit Manager",
  ],

  "Surgical Services": [
    "Operating Room",
    "CVOR",
    "Reconstructive Surgery",
    "Vascular Surgery",
  ],

  "Surgical Assistant": [
    "Certified Cardiac First Assist",
    "Certified Surgical First Assist",
    "CVOR Certified Surgical First Assist",
    "Surgical Assistant",
  ],

  "Certified Surgical Tech": [
    "Certified OR Tech / Surgical Tech (CST)",
    "Certified Ortho Tech",
    "CVOR Tech",
    "OR Tech / Surgical Tech",
    "Certified Surgical Tech",
  ],

  "Cath Lab Technologist": [
    "Ambulatory Cath Lab",
    "Electrophysiology (EP) Lab",
    "Electrophysiology Tech",
    "Cath Lab Tech",
    "Cath Lab Technologist",
  ],

  "Cardiac Tele": [
    "Med Surg Tele",
    "Cardiac PCU/IMC/Stepdown",
    "Telemetry",
    "Cardiac Tele",
  ],

  "Monitor Tech": [
    "EKG Tech",
    "Holter Monitor/Stress Test Technician",
    "Monitor Tech",
  ],

  "Radiology/Imaging": [
    "Diagnostic Imaging Assistant",
    "Interventional Radiology",
    "Interventional Radiology Tech",
    "Nuclear Medicine Tech",
    "Radiology/Imaging",
    "Registered Radiology Assistant",
    "Special Procedures",
    "X-Ray Tech",
  ],

  "MRI Tech": [
    "MRI Tech",
  ],

  "Mammography Technologist": [
    "Mammography Tech",
  ],

  "Ultrasound Tech": [
    "Ultrasound Tech - General",
    "Ultrasound Tech - OB/GYN",
    "Ultrasound Tech - Vascular",
    "Ultrasound Tech",
  ],

  "Perfusionist": [
    "Perfusionist",
  ],

  "Pharmacist": [
    "Pharmacist - Inpatient",
    "Pharmacist - Outpatient",
  ],

  "Pharmacy Tech": [
    "Pharmacy Tech Inpatient",
    "Pharmacy Tech Outpatient",
  ],

  "Physicist": [
    "Medical Physicist",
    "Physicist",
  ],

  "Radiation Therapist": [
    "Radiation Oncology",
    "Radiation Therapist",
  ],

  "Respiratory Therapy": [
    "Pulmonary Function Technologist",
    "Registered Respiratory Therapist",
    "Registered Respiratory Therapist - LTAC",
    "Registered Respiratory Therapist - NICU",
    "Registered Respiratory Therapist - Pediatrics",
    "Respiratory Therapy",
  ],

  "Occupational Therapy": [
    "Certified Occupational Therapist Assistant (COTA)",
    "COTA Acute Care",
    "COTA Behavioral Health",
    "COTA Home Health",
    "COTA Inpatient Rehab",
    "COTA LTAC",
    "COTA Outpatient",
    "COTA Pediatrics",
    "OT Acute Care",
    "OT Behavioral Health",
    "OT Home Health",
    "OT Inpatient Rehab",
    "OT LTAC",
    "OT Outpatient",
    "OT Pediatrics",
    "OT SNF",
    "Occupational Therapy",
  ],

  "Physical Therapy": [
    "Physical Therapist (PT)",
    "PT Acute Care",
    "PT Aide",
    "PT Behavioral Health",
    "PT Home Health",
    "PT Inpatient Rehab",
    "PT LTAC",
    "PT Outpatient",
    "PT Pediatrics",
    "PT SNF",
    "Physical Therapy",
  ],

  "PTA": [
    "Physical Therapist Assistant (PTA)",
    "PTA Acute Care",
    "PTA Behavioral Health",
    "PTA Home Health",
    "PTA Inpatient Rehab",
    "PTA LTAC",
    "PTA Outpatient",
    "PTA Pediatrics",
    "PTA SNF",
  ],

  "Speech Therapy": [
    "Speech-Language Pathologist",
    "SLP Acute Care",
    "SLP Behavioral Health",
    "SLP Home Health",
    "SLP Inpatient Rehab",
    "SLP LTAC",
    "SLP Outpatient",
    "SLP Pediatrics",
    "SLP SNF",
    "Speech Therapy",
  ],

  "Audiologist": [
    "Audiologist",
  ],

  "Social Services": [
    "Licensed Professional Clinical Counselor",
    "Licensed Professional Counselor",
    "Public Health",
    "School",
    "School Social Worker",
    "Social Services",
    "Social Worker Assistant",
  ],

  "Social Worker": [
    "Medical Social Worker",
    "Social Worker",
  ],

  "Behavioral Health": [
    "Behavioral Health Assistant",
    "Behavioral Health Tech",
    "Mental Health",
    "Behavioral Health",
  ],

  "Sports Medicine": [
    "Athletic Trainer",
    "Exercise Instructor",
    "Exercise Physiologist",
    "Kinesiotherapist",
    "Yoga Instructor",
    "Sports Medicine",
  ],

  "Patient Care Technician": [
    "Patient Care Technician",
  ],

  "CNA": [
    "CNA",
  ],

  "Medical Assistant": [
    "Medical Assistant",
    "Patient Care Coordinator",
  ],

  "Dentistry": [
    "Dental Assistant",
    "Pedodontist/Pediatric Dentist",
    "Periodontist",
    "Dentistry",
  ],

  "Dietary Services": [
    "Clinical Nutrition Manager",
    "Dietary Aide",
    "Dietetic Technician",
    "Dietitian",
    "Dietary Services",
  ],

  "EMT": [
    "EMS Educator",
    "EMT",
  ],

  "Paramedic": [
    "Paramedic",
  ],

  "Environmental Services": [
    "Environmental Services",
  ],

  "Finance/Accounting": [
    "Finance",
    "Finance/Accounting",
  ],

  "Human Resources": [
    "Administrative Assistant",
    "Human Resources",
  ],

  "Health and Information Management": [
    "Clinical Research",
    "Health Information Management",
    "Utilization Review",
    "Health and Information Management",
  ],

  "Lab Specialist": [
    "Lab Specialist",
  ],

  "Laboratory": [
    "Histology Tech",
    "Laboratory Assistant",
    "Medical Lab Scientist",
    "Medical Lab Technician",
    "Medical Technologist",
    "Metabolomic Technician",
    "Microbiology Tech",
    "Pathology Assistant",
    "Laboratory",
  ],

  "Phlebotomist": [
    "Phlebotomist",
  ],

  "Scrub Tech": [
    "Scrub Tech",
  ],

  "Mother Baby": [
    "L&D",
    "Mother Baby",
    "OB Tech",
    "Post-Partum",
  ],

  "Physician": [
    "Family Practice",
    "Internal Medicine",
    "OB/GYN",
    "Otorhinolaryngology (ENT)",
    "Physician",
  ],

  "Genetic Counselor": [
    "Genetic Counselor",
  ],

  "Dosimetrist": [
    "Dosimetrist",
  ],

  "Sitter": [
    "Sitter",
  ],
};

export const sortByOptions = [
  "Relevance",
  "Newest",
  "Highest Pay",
];